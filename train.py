...
import skimage
from dataloader.data_process_reno import inv_normalization,write_image
from dataloader.data_loader_reno import train_dataloader,valid_dataloader
from losses import losses
from tqdm import tqdm
import numpy as np


def _train_reno(model, args):
    ...
    # scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, args.num_epoch, eta_min=1e-5, last_epoch=-1)
   
    scheduler_cosine = optim.lr_scheduler.CosineAnnealingLR(optimizer, args.num_epoch, eta_min=6e-6)
        ...
        for iter_idx, batch_data in tqdm(enumerate(train_loader)):
            ...
            for i in range(int(H / Height)):  
                 for j in range(int(W/ Width)):
                    ...
                    loss1 = 1 - criterion(torch.clamp(pred_patch, 0, 1), label_patch, normalize = True)
